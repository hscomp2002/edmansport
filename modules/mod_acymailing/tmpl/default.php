		...
		<div class="acymailing_module_form" >
			...
			<table class="acymailing_form">
					...
					<?php foreach($fieldsToDisplay as $oneField){
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							<td class="acyfield_<?php echo $oneField; ?>">
								...
								<input id="user_name_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" '; echo $placeholder ?> class="inputbox" type="text" name="user[name]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->name; ?>" />
							</td> <?php
							...
							<td class="acyfield_<?php echo $oneField; ?>">
								...
								<input id="user_email_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" '; echo $placeholder ?> class="inputbox" type="text" name="user[email]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->email; ?>" />
							</td> <?php
							...
							<?php if(!empty($identifiedUser->userid) AND in_array($oneField,array('name','email'))){
									$placeholder = '';
									if(!empty($extraFields[$oneField]->fieldname) && !$displayOutside) $placeholder = 'placeholder="' . JText::_($extraFields[$oneField]->fieldname) . '"'; ?>
				...
				if(empty($identifiedUser->userid) AND $config->get('captcha_enabled') AND acymailing_level(1)){ ?>
					...
					$refreshImg = '<span class="refreshCaptchaModule" onClick="refreshCaptchaModule()">&nbsp;</span>';
