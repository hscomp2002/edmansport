...
' . $template->body . '
		...
		if(!JFolder::exists($jpath)){
			if(!JFolder::create($jpath)){
				acymailing_display('Error creating temp folder in directory: '.$jpath,'error');
				return false;
			}
		}
		if(!JFolder::create($tmp_dest)){
			acymailing_display('Error creating folder in temp directory: '.$tmp_dest,'error');
			return false;
		}
		...
		if(!empty($template->thumb)){
			...
			if(!$resCopyThumb){
				acymailing_display('Error copying the thumb picture','warning');
			}
		}
		...
		if(!$resCopyIndex){
			acymailing_display('Error copying the file index.html to temp directory '.$tmp_dest,'error');
			return false;
		}
		$zipFilesArray = array();
