			...
			<div class="acymailing_form">
					<?php foreach($fieldsToDisplay as $oneField){
						...
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							$placeholder = '';
							if(!$displayOutside) $placeholder = 'placeholder="' . $nameCaption . '"'; ?>
								<span class="acyfield_<?php echo $oneField; ?>"><input id="user_name_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" '; echo $placeholder; ?> class="inputbox" type="text" name="user[name]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->name; ?>" /></span>
							<?php
							...
							$placeholder = '';
							if(!$displayOutside) $placeholder = 'placeholder="' . $emailCaption . '"'; ?>
								<span class="acyfield_<?php echo $oneField; ?>"><input id="user_email_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" '; echo $placeholder; ?> class="inputbox" type="text" name="user[email]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->email; ?>" /></span>
							<?php
							...
							<?php if(!empty($identifiedUser->userid) AND in_array($oneField,array('name','email'))){
									$placeholder = '';
									if(!empty($extraFields[$oneField]->fieldname)) $placeholder = 'placeholder="' . JText::_($extraFields[$oneField]->fieldname) . '"'; ?>
									<input id="user_<?php echo $oneField; ?>_<?php echo $formName; ?>" disabled="disabled" class="inputbox" type="text" name="user[<?php echo $oneField;?>]" <?php echo $sizestyle; echo $placeholder; ?> value="<?php echo @$identifiedUser->$oneField; ?>"/>
							<?php }else{
				...
				if(empty($identifiedUser->userid) AND $config->get('captcha_enabled') AND acymailing_level(1)){ ?>
					...
					<span class="refreshCaptchaModule" onClick="refreshCaptchaModule()">&nbsp;</span>
