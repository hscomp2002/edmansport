			...
			?>
				<tr class="<?php echo "row$k"; ?>">
					...
					<td>
						...
						<?php if($this->app->isAdmin()){
							echo acymailing_tooltip('<b>'.JText::_('JOOMEXT_ALIAS').' : </b>'.$row->alias, ' ', '', $row->subject,acymailing_completeLink('newsletter&task=edit&mailid='.$row->mailid));
						} else{
							echo acymailing_tooltip('<b>'.JText::_('JOOMEXT_ALIAS').' : </b>'.$row->alias, ' ', '', $row->subject,acymailing_completeLink('frontnewsletter&task=edit&mailid='.$row->mailid));
						}?>
					</td>
					<td align="center">
