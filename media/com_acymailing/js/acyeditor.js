...
function Initialisation(id, type, urlBase, urlAdminBase, cssUrl, forceComplet, modeList, modeTemplate, modeArticle, joomla2_5, joomla3, back, tagAllowed, texteSuppression, titleSuppression, titleEdition, titleTemplateDelete, titleTemplateText, titleTemplatePicture, titleShowAreas) {
	...
	if (jQuery('#AcyLienImage')[0] == null || jQuery('#AcyLienImage')[0] == undefined)
	{
		...
		if(inPopup)
				endUrl='&inpopup=true';
		lienImage.href = "index.php?option=com_acymailing&tmpl=component&ctrl=editor&task=browse&e_name=ACY_NAME_TO_REPLACE&image_zone=true" + endUrl;
		if (!isBack){
			lienImage.href = "index.php?option=com_acymailing&tmpl=component&ctrl=fronteditor&task=browse&e_name=ACY_NAME_TO_REPLACE&image_zone=true" + endUrl;
		}
	...
	if (jQuery('#AcyLienMediaBrowser')[0] == null || jQuery('#AcyLienMediaBrowser')[0] == undefined)
	{
			...
			if(inPopup)
				endUrl='&inpopup=true';
			lienMediaBrowser.href = "index.php?option=com_acymailing&tmpl=component&ctrl=editor&task=browse&e_name=ACY_NAME_TO_REPLACE" + endUrl;
			if (!isBack)
			{
				lienMediaBrowser.href = urlBase + "index.php?option=com_acymailing&tmpl=component&ctrl=fronteditor&task=browse&e_name=ACY_NAME_TO_REPLACE" + endUrl;
			}
	...
	else
	{
		...
		if (!acyeditor_listmode && isTagAllowed)
		{
			...
			toolbarGroupsCKEditor.push({ name: 'insert', items: [ "acymediabrowser", "addtag" ]});
		}
		else
		{
			toolbarGroupsCKEditor.push({ name: 'insert', items: [ "acymediabrowser" ]});
		}
...
function insertImageTag(tag){
	try{
		jInsertEditorText(tag,'edition_en_cours');
		return true;
	}catch(err){
		try{
