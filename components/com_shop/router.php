<?php
/**
 * @version		$Id:router.php 1 2015-03-23Z  $
 * @package		Shop
 * @subpackage 	Router
...
require_once(JPATH_ADMINISTRATOR.'/components/com_shop/helpers/shop.php');
...
defined('_JEXEC') or die('Restricted access');
  ...
  function ShopBuildRoute( &$query )
  {
	...
	{
		$view = $query['view'];
		if (empty($query['Itemid']) || empty($menuItem) || $menuItem->component != 'com_shop')
		{
			$segments[] = $query['view'];
			unset($query['view']);
		}
	...
	if (isset($view) && in_array($view, $listviews))
	{
		...
		if ((isset($query['id']) && ($mId != (int) $query['id'])) || $mView != $view)
		{
			...
			$categories = JCategories::getInstance('Shop', $options);
