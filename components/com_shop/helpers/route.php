...
abstract class ShopHelperRoute
{
	...
	/**
	 * @param	int	The route of the shop
	 */
	public static function getShopRoute($id, $catid)
	{
		$needles = array(
			'factor'  => array((int) $id)
		);
		//Create the link
		$link = 'index.php?option=com_shop&view=factor&id='. $id;
		if ($catid > 1) {
			$categories = JCategories::getInstance('Factor');
		...
		if ($item = ShopHelperRoute::_findItem($needles)) {
			$link .= '&Itemid='.$item;
		};
	...
	public static function getCategoryRoute($catid, $language = 0)
	{
		...
		$catviews = ShopHelper::getCategoryViews();
		...
		{
			//Create the link
			$link = 'index.php?option=com_shop&view='.$view.'&category='.$category->slug;
