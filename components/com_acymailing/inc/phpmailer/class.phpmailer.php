...
class acymailingPHPMailer {
	...
	public function Send() {
		...
		} catch (acymailingphpmailerException $e) {
			$this->mailHeader = '';
	...
	protected function SmtpSend($header, $body) {
		...
		if(!$this->smtp->Mail($smtp_from)) {
			...
			$this->SetError(empty($errorTmp) ? $this->Lang('from_failed').$smtp_from : implode(', ',$errorTmp));
		...
		if ($this->IsError()) {
			...
			try {
				if (!defined('PKCS7_TEXT')) {
						throw new acymailingphpmailerException($this->Lang('signing').' OpenSSL extension missing.');
				}
	...
	protected function SetError($msg) {
		...
		if ($this->Mailer == 'smtp' and !is_null($this->smtp)) {
			...
			if (!empty($lasterror) and array_key_exists('smtp_msg', $lasterror)) {
				$errorCode = (array_key_exists('smtp_code',$lasterror)?$lasterror['smtp_code'] . ', ':'');
				$msg .= '<p>' . $this->Lang('smtp_error') . $errorCode . $lasterror['smtp_msg'] . "</p>\n";
			}
		}
		$this->ErrorInfo = $msg;
	}
	...
	public function set($name, $value = '') {
		try {
			if (isset($this->$name) ) {
				...
				throw new acymailingphpmailerException($this->Lang('variable_set') . $name, self::STOP_CRITICAL);
			}
		} catch (Exception $e) {
			$this->SetError($e->getMessage());
