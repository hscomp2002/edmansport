...
class plgAcymailingStats extends JPlugin
{
	...
	function acymailing_replaceusertags(&$email,&$user,$send = true){
		...
		if(!$email->sendHTML OR empty($email->type) OR !in_array($email->type,array('news','autonews','followup','welcome','unsub')) OR strpos($email->body,'{nostatpicture}')){
		...
		if(empty($user->subid) || !$send){
			...
			$pictureLink = acymailing_frontendLink('index.php?option=com_acymailing&ctrl=stats&mailid='.$email->mailid.'&subid='.$user->subid.'&no_html=1'.$item);
		}
